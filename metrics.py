...
def get_iou(mask_name,predict, target2, pred):
    ...
    print('%s:iou=%f,%d,%d' % (mask_name, iou_tem, target2, pred))

    return iou_tem
def get_iou1(mask_name,predict,label_var,output_cla):
    image_mask = cv2.imread(mask_name,0)
    if np.all(image_mask == None):
        image_mask = imageio.mimread(mask_name)
        image_mask = np.array(image_mask)[0]
    ...
    # Iou.add_batch(predict, image_mask)
    # a, b, c, d, e= Iou.evaluate()
    print('%s:iou=%f,truth=%d, pred=%d' % (mask_name, iou_tem, label_var, output_cla))

    return iou_tem
def get_iou2(mask_name,predict):
    image_mask = cv2.imread(mask_name,0)
    if np.all(image_mask == None):
        image_mask = imageio.mimread(mask_name)
        image_mask = np.array(image_mask)[0]
